...
namespace Chromaria
{
    ...
    public class Creature : RotateableImage
    {
        ...
        public void checkBounds(GameTime time)
        {
			...
			while (boundsNW.X <= 0 || boundsNW.Y <= 0 || (Chromaria.Simulator.fixedSizeWorld && (boundsSE.X >= Chromaria.Simulator.initialBoardWidth - 1 || boundsSE.Y >= Chromaria.Simulator.initialBoardHeight - 1))) 
			{
        ...
        public bool isAtValidPlantingLocation()
        {
            ...
            if (!Simulator.startFromCenter && Chromaria.MainLoop.numUpdates < 25)
                return false;
            ...
            if (Simulator.startFromCenter && Chromaria.Simulator.useHarderPlantingFunction)
            {
