...
namespace Chromaria
{
    public class SensorField
    {
        ...
        public SensorField(int spriteResX, int spriteResY, int segNum, int segLength, Simulator sim)
        {
            ...
            Chromaria.Simulator.FieldTexture = new Texture2D(sim.GraphicsDevice, SegmentLength * ResolutionX, SegmentLength * ResolutionY);
            Color[] sensorPixels = new Color[Chromaria.Simulator.FieldTexture.Width * Chromaria.Simulator.FieldTexture.Height];
            ...
            for (int y = 0; y < SegmentLength * ResolutionY; y++)
            {
                for (int x = 0; x < SegmentLength * ResolutionX; x++)
                {
                    ...
                    if (x % ResolutionX != 0  || y % ResolutionY != 0)
                        sensorPixels[x + (Chromaria.Simulator.FieldTexture.Width * y)] = Color.Transparent;

                    // Otherwise, give the pixel a solid color.
                    else
                    {
                        sensorPixels[x + (Chromaria.Simulator.FieldTexture.Width * y)] = Color.DarkViolet;
                        SensorArray[x / ResolutionX, y / ResolutionY] = new Point(x, y);
                    }
                }
            }
            ...
            Chromaria.Simulator.FieldTexture.SetData(sensorPixels);

            // Initialize the texture for onscreen visualization of sensor content, which may or may not actually be used
            Chromaria.Simulator.sensorContentsTexture = new Texture2D(sim.GraphicsDevice, SegmentLength * ResolutionX, SegmentLength * ResolutionY);
        }
        ...
        public void Update(Creature creature, RotationPacket creaturePacket)
        {
            ...
            Color[] sensorContentsPixels = new Color[Chromaria.Simulator.FieldTexture.Width * Chromaria.Simulator.FieldTexture.Height];
            for (int row = 0; row < Chromaria.Simulator.FieldTexture.Height; row++)
            {
                for (int col = 0; col < Chromaria.Simulator.FieldTexture.Width; col++)
                    sensorContentsPixels[col + (row * Chromaria.Simulator.FieldTexture.Width)] = Color.Transparent;
            }
                    ...
                    for (int i = 0; i < intersectingImages.Count; i++)
                    {
                        ...
                        if(globalSensorCoord.X >= imageNW.X && globalSensorCoord.X < imageSE.X && globalSensorCoord.Y >= imageNW.Y && globalSensorCoord.Y < imageSE.Y)
                        {
                            ...
                            if (worldTexel.A != 0)
                            {
                                ...
                                if (Chromaria.Simulator.drawSensorField && (!Chromaria.Simulator.depthTest || creature.ID==Chromaria.Simulator.manuallyControlledCreatureID))
            ...
            if (Chromaria.Simulator.drawSensorField)
                Chromaria.Simulator.sensorContentsTexture.SetData(sensorContentsPixels);
        ...
        private List<Image> updateIntersectingImages(Image creature, RotationPacket creaturePacket)
        {
            ...
            indicesOfPossibleNeighbors.AddRange(Chromaria.Simulator.regions[creaturePacket.NWCoord.Y / Chromaria.Simulator.regionHeight, creaturePacket.NWCoord.X / Chromaria.Simulator.regionWidth]);
            indicesOfPossibleNeighbors.AddRange(Chromaria.Simulator.regions[creaturePacket.NWCoord.Y / Chromaria.Simulator.regionHeight, creaturePacket.SECoord.X / Chromaria.Simulator.regionWidth]);
            indicesOfPossibleNeighbors.AddRange(Chromaria.Simulator.regions[creaturePacket.SECoord.Y / Chromaria.Simulator.regionHeight, creaturePacket.NWCoord.X / Chromaria.Simulator.regionWidth]);
            indicesOfPossibleNeighbors.AddRange(Chromaria.Simulator.regions[creaturePacket.SECoord.Y / Chromaria.Simulator.regionHeight, creaturePacket.SECoord.X / Chromaria.Simulator.regionWidth]);
