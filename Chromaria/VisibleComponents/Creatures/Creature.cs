...
namespace Chromaria.VisibleComponents.Creatures
{
    ...
    public class Creature : RotateableImage
    {
        ...
        Vector2 rotatedCenterAsVector, creatureCenter, oldPosition, newPosition, boundsNW, boundsSE, creatureNW, creatureNE, creatureSE, creatureSW, sensorNW, sensorNE, sensorSW, sensorSE,
        ...
        public float BlackRatio { get { return blackRatio; } }
        public float WhiteRatio { get { return whiteRatio; } }
        public float RedRatio { get { return redRatio; } }
        public float GreenRatio { get { return greenRatio; } }
        public float BlueRatio { get { return blueRatio; } }
        public float YellowRatio { get { return yellowRatio; } }
        public float MagentaRatio { get { return magentaRatio; } }
        public float CyanRatio { get { return cyanRatio; } }
        ...
        public Creature(Texture2D bodyTexture, float x, float y, float headingInRadians, Simulator sim, bool drawSensorField_in, bool trackPlanting_in, int numSensors, GameTime gameTime = null)
        ...
        public override void Draw(GameTime gameTime)
        {
            ...
            if (drawSensorField && (!Simulator.depthTest || ID==Simulator.manuallyControlledCreatureID))
            {
                // Draw the sensor field
                fieldNWGlobalCoords = new Vector2(Position.X + SensorFieldAnchorPoint.X - (Simulator.FieldTexture.Width / 2), Position.Y + SensorFieldAnchorPoint.Y - (Simulator.FieldTexture.Height / 2));
                rotationOrigin = new Vector2(Simulator.FieldTexture.Width / 2, Simulator.FieldTexture.Height * 1.5f);
                Simulator.spriteBatch.Draw(Simulator.FieldTexture, fieldNWGlobalCoords + rotationOrigin, null, Color.White, XNAHeading, rotationOrigin, 1.0f, SpriteEffects.None, Depth);
                
                // Draw the sensor field's contents
                Simulator.spriteBatch.Draw(Simulator.sensorContentsTexture, new Vector2(100, 300), null, Color.White, 0.0f, Vector2.Zero, 1.0f, SpriteEffects.None, 0.0f);
            }

            // If desired, draw the red texture overtop of the creature's texture if it is planting
            if (trackPlanting && currentState == Simulator.State.Planting)
            {
                center = new Vector2(Texture.Width / 2, Texture.Height / 2);
                Simulator.spriteBatch.Draw(Simulator.redTexture, Position + center, null, Color.White, (float)Heading, center, 1.0f, SpriteEffects.None, 0.0f);
            }
        }
        ...
        public void checkregions()
        {
            ...
            deltaX = (packetBeforeMoving.NWCoord.X % Simulator.regionWidth) - (packetAfterMoving.NWCoord.X % Simulator.regionWidth);
            if (Math.Abs(deltaX) > Simulator.deltaXThreshold)
            {
                // Moving from left to right
                if (deltaX > 0)
                {
                    Simulator.regions[packetBeforeMoving.NWCoord.Y / Simulator.regionHeight, packetBeforeMoving.NWCoord.X / Simulator.regionWidth].Remove(Simulator.indexOfCurrentCreature);
                    if (packetAfterMoving.NWCoord.Y % Simulator.regionHeight > packetAfterMoving.SECoord.Y % Simulator.regionHeight)
                        Simulator.regions[packetBeforeMoving.SECoord.Y / Simulator.regionHeight, packetBeforeMoving.NWCoord.X / Simulator.regionWidth].Remove(Simulator.indexOfCurrentCreature);
                }
                // Moving from right to left
                else
                {
                    Simulator.regions[packetAfterMoving.NWCoord.Y / Simulator.regionHeight, packetAfterMoving.NWCoord.X / Simulator.regionWidth].Add(Simulator.indexOfCurrentCreature);
                    if (packetAfterMoving.NWCoord.Y % Simulator.regionHeight > packetAfterMoving.SECoord.Y % Simulator.regionHeight)
                        Simulator.regions[packetAfterMoving.SECoord.Y / Simulator.regionHeight, packetAfterMoving.NWCoord.X / Simulator.regionWidth].Add(Simulator.indexOfCurrentCreature);
                }
            }

            // Check if the right edge of the texture has crossed a region border
            deltaX = (packetBeforeMoving.SECoord.X % Simulator.regionWidth) - (packetAfterMoving.SECoord.X % Simulator.regionWidth);
            if (Math.Abs(deltaX) > Simulator.deltaXThreshold)
            {
                // Moving from left to right
                if (deltaX > 0)
                {
                    Simulator.regions[packetAfterMoving.NWCoord.Y / Simulator.regionHeight, packetAfterMoving.SECoord.X / Simulator.regionWidth].Add(Simulator.indexOfCurrentCreature);
                    if (packetAfterMoving.NWCoord.Y % Simulator.regionHeight > packetAfterMoving.SECoord.Y % Simulator.regionHeight)
                        Simulator.regions[packetAfterMoving.SECoord.Y / Simulator.regionHeight, packetAfterMoving.SECoord.X / Simulator.regionWidth].Add(Simulator.indexOfCurrentCreature);
                }
                // Moving from right to left
                else
                {
                    Simulator.regions[packetBeforeMoving.NWCoord.Y / Simulator.regionHeight, packetBeforeMoving.SECoord.X / Simulator.regionWidth].Remove(Simulator.indexOfCurrentCreature);
                    if (packetAfterMoving.NWCoord.Y % Simulator.regionHeight > packetAfterMoving.SECoord.Y % Simulator.regionHeight)
                        Simulator.regions[packetBeforeMoving.SECoord.Y / Simulator.regionHeight, packetBeforeMoving.SECoord.X / Simulator.regionWidth].Remove(Simulator.indexOfCurrentCreature);
                }
            }

            // Check if the top edge of the texture has crossed a region border
            deltaY = (packetBeforeMoving.NWCoord.Y % Simulator.regionHeight) - (packetAfterMoving.NWCoord.Y % Simulator.regionHeight);
            if (Math.Abs(deltaY) > Simulator.deltaYThreshold)
            {
                // Moving from top to bottom (y is increasing overall, though, as it is measured from top=0)
                if (deltaY > 0)
                {
                    Simulator.regions[packetBeforeMoving.NWCoord.Y / Simulator.regionHeight, packetBeforeMoving.NWCoord.X / Simulator.regionWidth].Remove(Simulator.indexOfCurrentCreature);
                    if (packetAfterMoving.NWCoord.X % Simulator.regionWidth > packetAfterMoving.SECoord.X % Simulator.regionWidth)
                        Simulator.regions[packetBeforeMoving.NWCoord.Y / Simulator.regionHeight, packetBeforeMoving.SECoord.X / Simulator.regionWidth].Remove(Simulator.indexOfCurrentCreature);
                }
            ...
            deltaY = (packetBeforeMoving.SECoord.Y % Simulator.regionHeight) - (packetAfterMoving.SECoord.Y % Simulator.regionHeight);
            if (Math.Abs(deltaY) > Simulator.deltaYThreshold)
            {
                // Moving from top to bottom (y is increasing overall, though, as it is measured from top=0)
                if (deltaY > 0)
        ...
        /// </summary>
        /// <param name="time">GameTime manager from the main game engine.</param>
        public void checkBounds(GameTime time)
        {
			...
			while (boundsNW.X <= 0 || boundsNW.Y <= 0 || (Simulator.fixedSizeWorld && (boundsSE.X >= Simulator.initialBoardWidth - 1 || boundsSE.Y >= Simulator.initialBoardHeight - 1))) 
			{
				oldPosition = newPosition;

				// Check the western border
				if (boundsNW.X <= 0) {
					newPosition = new Vector2 (oldPosition.X + 20, newPosition.Y);
					speed = Vector2.Zero;
				}

				// Check the northern border
				if (boundsNW.Y <= 0) {
					newPosition = new Vector2 (newPosition.X, oldPosition.Y + 20);
					speed = Vector2.Zero;
				}

				if (Simulator.fixedSizeWorld) {
					// Check the eastern border (if applicable)
					if (boundsSE.X >= Simulator.initialBoardWidth - 1) {
						newPosition = new Vector2 (oldPosition.X - 20, newPosition.Y);
						speed = Vector2.Zero;
					}

					// Check the southern border (if applicable)
					if (boundsSE.Y >= Simulator.initialBoardHeight - 1) {
						newPosition = new Vector2 (newPosition.X, oldPosition.Y - 20);
						speed = Vector2.Zero;
					}
				}

				// Recalculate the counds based on the new position
				boundsNW = new Vector2(newPosition.X + Texture.Width / 2 - distCenterToBorder, newPosition.Y + Texture.Height / 2 - distCenterToBorder);
				boundsSE = new Vector2(newPosition.X + Texture.Width / 2 + distCenterToBorder, newPosition.Y + Texture.Height / 2 + distCenterToBorder);
			}
        ...
        {
            // Optionally prevent creatures from planting where they are born
            if (!Simulator.startFromCenter && MainLoop.numUpdates < 25)
                return false;
            ...
            if (Simulator.startFromCenter && Simulator.useHarderPlantingFunction)
            {
                ...
                float distanceFromCenter = Vector2.Distance(Position + rotatedCenterAsVector, new Vector2(Simulator.initialBoardWidth / 2, Simulator.initialBoardHeight / 2));
                if (distanceFromCenter < (Simulator.harderPlantingFunctionMultiplier * Simulator.pixelHeight))
                    return false;
            }
			...
			return (summedDifferences <= Simulator.toleratedDifference);
        ...
        public virtual void reset(float newXNAHeading)
        {
            ...
            Position = new Vector2(Simulator.initialBoardWidth / 2, Simulator.initialBoardHeight / 2);
