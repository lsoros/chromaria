...
namespace Chromaria.SharpNeatLib.NeatGenome
{
	...
	//		{
                ...
                //network = GenomeDecoder.DecodeToCTRNN(this);
    //        }

			return network;
		}

        public INetwork DecodeToCTRNN()
        {
            network = GenomeDecoder.DecodeToCTRNN(this);
            return network;
        }
		...
		private void Mutate_AddNode(EvolutionAlgorithm ea)
		{
            ...
            for (int attempts = 0; attempts < 5; attempts++)
            {
                ...
                {	// No existing matching structure, so generate some new ID's.
                    ...
                    newLayer = (neuronGeneList.GetNeuronById(connectionToReplace.SourceNeuronId).Layer + neuronGeneList.GetNeuronById(connectionToReplace.TargetNeuronId).Layer) / 2.0f;
...
//                  NeuronGene targetNeuron = neuronGeneList[tgtNeuronIdx];
					...
					if(!TestForExistingConnection(sourceId, targetId) && (ea.NeatParameters.recurrenceDisabled && !IsRecurrentConnection(sourceId, targetId)))
					{
...
//		}
        ...
        private bool IsRecurrentConnection(uint sourceId, uint targetId)
        {
            // This check will necessarily also fail if the source ID and target ID are the same
            if (neuronGeneList.GetNeuronById(sourceId).Layer >= neuronGeneList.GetNeuronById(targetId).Layer)
                return true;
            else
                return false;
        }
