...
using Chromaria.SharpNeatLib.Multiobjective;
...
namespace Chromaria.SharpNeatLib.Evolution
{
	public class EvolutionAlgorithm
	{
        ...
        IdGenerator nonGenerationalIdGen;
		...
		/// Default Constructor.
		/// </summary>
		public EvolutionAlgorithm(Population pop, IPopulationEvaluator populationEvaluator, NeatParameters neatParameters, uint nextGenomeID = 0, uint nextInnovationID = 0)
		{
			this.pop = pop;
            ...
            else if (neatParameters.notGenerational)
            {
                nonGenerationalIdGen = new IdGenerator(nextGenomeID, nextInnovationID);
            }
            else if (!neatParameters.notGenerational)
            {
                InitialisePopulation();
		...
		public uint NextGenomeId
		{
			get
			{
                if (neatParameters.notGenerational)
                    return nonGenerationalIdGen.NextGenomeId;
                else
		...
		public uint NextInnovationId
		{
			...
			{
                if (neatParameters.notGenerational)
                    return nonGenerationalIdGen.NextInnovationId;
                else
...
//		}
		...
		private void DetermineSpeciesTargetSize()
		{
			...
			foreach(Species species in pop.SpeciesTable.Values)
			{
				...
				// Calculate how many elite genomes to keep in the next Chromaria.Simulator.round. If this is a large number then we can only
