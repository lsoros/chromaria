...
namespace Chromaria.Utils
{
    ...
    public class MathHelpers
    {
        ...
        public static Point ConvertToCartesian(int r, int theta)
        {
            ...
            Point XY = new Point();
            XY.X = Convert.ToInt32(Math.Floor(Math.Cos(radians) * r + (Simulator.pixelHeight / 2)));
            XY.Y = Convert.ToInt32(Math.Floor(Math.Sin(radians) * r + (Simulator.pixelHeight / 2)));
            return XY;
        }
        ...
        {
            Point XY = new Point();
            XY.X = Convert.ToInt32((float)Math.Cos(radians) * r + (Simulator.pixelHeight / 2));
            XY.Y = Convert.ToInt32((float)Math.Sin(radians) * r + (Simulator.pixelHeight / 2));
            return XY;
        }
